...
public class SeaPortProgram extends JFrame {
	...
	private DefaultMutableTreeNode gTN(final String world) {
		...
		for (final SeaPort s : this.world.gP()) {
			port = new DefaultMutableTreeNode("SeaPort: " + s.getName());
			head.add(port);
			port.add(bN(s.gD(), "Docks"));
			port.add(cN(s.gQ(), "Queue"));
			port.add(cN(s.gS(), "Ships"));
			port.add(cN(s.gP(), "Persons"));
		}
		return head;
	}
	...
	private MutableTreeNode bN(final ArrayList<Dock> a, final String s1) {
		...
		for (final Iterator<Dock> iterator = a.iterator(); iterator.hasNext();) {
			...
			String temp;
			temp = d.returnShip().get(0) != null ? "Ship: " + d.returnShip().get(0).getName() : "(Empty)";
			attach(head, temp);
		}
		return head;
	}
	...
	private void parse(final String line) {
		...
		while (sc.hasNext()) {
			...
			if ("port".equals(next)) {
				world.aP(new SeaPort(sc));
			} else if ("dock".equals(next)) {
				world.aD(new Dock(sc));
			} else if ("ship".equals(next)) {
				world.aS(new Ship(sc));
			} else if ("pship".equals(next)) {
				world.aS(new PassengerShip(sc));
			} else if ("cship".equals(next)) {
				world.aS(new CargoShip(sc));
			} else if ("person".equals(next)) {
				world.aPer(new Person(sc));
			}
		}
	}
	...
	private void s(final String t, final String choice) {
		...
		if (output.getText().isEmpty()) {
			...
			final ArrayList<SeaPort> ports = world.gP();
			if ("Name".equals(t)) {
				try {
	...
	private void sort(final String selection, final String choice) {
		for (final Iterator<SeaPort> iterator2 = world.gP().iterator(); iterator2.hasNext();) {
			final SeaPort port = iterator2.next();
			comparator(port);
			port.gQ().sort(new QueueComparator(selection, choice));
			output.setText(world.toString());
		}
	}

	private void comparator(final SeaPort port) {
		port.gD().sort(new NameComparator());
		port.gP().sort(new NameComparator());
		port.gS().sort(new NameComparator());
	}
	...
	private void storePortDataByIndex(final int i, final ArrayList<SeaPort> ports, final StringBuilder r) {
		for (final Iterator<SeaPort> iterator = ports.iterator(); iterator.hasNext();) {
			...
			final ArrayList<Dock> d = port.gD();
			final ArrayList<Ship> s = port.gS();
			final ArrayList<Ship> q = port.gQ();
			final ArrayList<Person> p = port.gP();
			fDI(i, d, r);
			fSI(i, s, r);
			fSI(i, q, r);
			fPI(i, p, r);
